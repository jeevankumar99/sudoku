...
/* changing color of user input is done
   checking for win is incomplete
   Undo as ctrl+z and U is incomplete
   edit cases 1-9 to include . backspace and 0.
*/
...
int main(int argc, char *argv[])
{
    ...
    // Let the user play
    int ch;
    int tempx = 0, tempy = 0, temp_val = 0;
    bool user_inputs[9][9];
    do
    {
        // Refresh the screen
        refresh();
        ...
        switch (ch)
        {
            ...
            case KEY_BACKSPACE:
            case KEY_DC:
                if (g.board[g.y][g.x] != 0 && user_inputs[g.y][g.x] == false)
                {
                    break;
                }
            ...
            case 'U':
            case CTRL('z'):
                g.board[tempy][tempx] = 0;
                draw_numbers();
                show_cursor();
                break;
        }
        int check_win[9];
        int repeat = 0, counter = 9;
        bool row = false, col = false;
        for (int i = 0; i < 9; i++)
        {
            for (int j = 0; j < 9; j++)
            {
                if (g.board[i][j] <= 9 && g.board[i][j] > 0)
                {
                    for (int k = 0; k < j; k++)
                    {
                        if (g.board[i][j] == check_win[k])
                        {
                            repeat++;
                        }
        ...
        for (int i = 0; i < 9; i++)
        {
            for (int j = 0; j < 9; j++)
            {
                if (g.board[j][i] <= 9 && g.board[j][i] > 0)
                {
                    for (int k = 0; k < j; k++)
                    {
                        if (g.board[j][i] == check_win2[k])
                        {
                            repeat++;
                        }
        ...
        if (row == true && col == true)
        {
            move(4, 4);
            printw("You Have Won");
            init_pair(PAIR_DIGITS, COLOR_GREEN, BG_DIGITS);
            timeout(5000);
            exit(0);
        }
...
void draw_user_numbers(void)
{
    if (has_colors())
    {
        attron(COLOR_PAIR(USER_DIGITS));
    }

    // Iterate over board's numbers
    char d = (g.board[g.y][g.x] == 0) ? '.' : g.board[g.y][g.x] + '0';
    mvaddch(g.top + g.y + 1 + g.y / 3, g.left + 2 + 2 * (g.x + g.x / 3), d);
    refresh();
...
bool startup(void)
{
    ...
    if (has_colors())
    {
        ...
        if (init_pair(PAIR_BANNER, FG_BANNER, BG_BANNER) == ERR ||
            ...
            init_pair(USER_DIGITS, COLOR_WHITE, BG_DIGITS) == ERR)
        {
            endwin();
            return false;
        }
